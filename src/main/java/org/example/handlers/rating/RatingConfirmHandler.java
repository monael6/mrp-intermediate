...
public class RatingConfirmHandler implements HttpHandler {
    ...
    private void handleInternal(HttpExchange exchange) throws Exception {
        ...
        boolean ok = RatingRepository.confirmComment(id, user.id);
        if (!ok) {
            // rating gehört dir nicht oder existiert nicht
            exchange.sendResponseHeaders(403, -1);
            return;
        }

        String res = "{\"message\":\"comment confirmed\"}";
